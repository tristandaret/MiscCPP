   ...
   Process::Process()
{
   ...
   for (int i = 0; i < nddbins; i++) {
      ...
      vdd_fph1f_WF.push_back(new TH1F(
         ...
         Form("Energy loss | %d < drift distance < %d; dE/dx (ADC counts/cm); Count", ddmin, ddmax), 100, 0, xmax));
   ...
   for (int i = 0; i < nphibins; i++) {
      ...
      vphi_fph1f_WF.push_back(
         new TH1F(Form("fph1f_phi_WF_%d_%d", phimin, phimax),
                  Form("Energy loss | %d < #varphi < %d; dE/dx (ADC counts/cm); Count", phimin, phimax), 100, 0, xmax));
   ...
   for (int i = 0; i < nthetabins; i++) {
      ...
      vtheta_fph1f_WF.push_back(new TH1F(
         ...
         Form("Energy loss | %d < #theta < %d; dE/dx (ADC counts/cm); Count", thetamin, thetamax), 100, 0, xmax));
...
void Process::Run()
{
   ...
   // apmcutlow = 2, apmcuthigh = 4;		fcutslist += ("_" + std::to_string(apmcutlow)	+ "apm" +
   ...
   // std::to_string(tcutmax));
   // nclmin=50, nclmax = 150;				fcutslist += ("_" + std::to_string(nclmin)		+ "ncl" +
   // std::to_string(nclmax)); dxmin = 50, dxmax = 150;			fcutslist += ("_" + std::to_string(dxmin) 		+ "dx" +
   // std::to_string(dxmax)); pullmumax = 2;						fcutslist += ("_pullmu" + std::to_string(pullmumax)); phimin
   // = 45; phimax = 90;			fcutslist += ("_" + std::to_string(phimin)		+ "phi" + std::to_string(phimax));
   // thetamin = 0; thetamax = 45;			fcutslist += ("_" + std::to_string(thetamin)	+ "theta" +
   // std::to_string(thetamax));
   chi2max = 5;
   fcutslist += ("_chi2ndf" + std::to_string(chi2max));
   dxmin = 25;
   fcutslist += ("_" + std::to_string(dxmin) + "dx");
   momcutlow = 250, momcuthigh = 500;

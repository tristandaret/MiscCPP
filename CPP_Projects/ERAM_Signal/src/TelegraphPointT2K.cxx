...
double Qpad(double xpad, double ypad, double xcharge, double ycharge, double RC, double t, double Qdrop){
    ...
    double erfx =				std::erf((xright-xcharge) / (sigma*std::sqrt(2))) - std::erf((xleft-xcharge) / (sigma*std::sqrt(2)));
    double erfy =				std::erf((yhigh- ycharge) / (sigma*std::sqrt(2))) - std::erf((ylow- ycharge) / (sigma*std::sqrt(2)));

...
void TrackModel::SetParameters(const double &length, const double &impact, const double &phi, const double &RC,
...
void TrackModel::ComputeAmplitudeLoss(const double &length, const double &impact, const double &phi, const double &RC,
                                      const double &drift, const double &Dt)
{
...
Double_t TrackModel::dETFdt(Double_t *x, Double_t *par)
{
   ...
   if (t <= 0)
      ...
      return fnormelec * (-ws * std::exp(-ws * t) +
...
Double_t TrackModel::Charge(Double_t *x, Double_t *par)
{
   ...
   // Return result
   return std::sqrt(1 + fm * fm) / (2 * fm) *
          (coeff1 * (expRC1 - expRC2 + expRC3 - expRC4) + erfRC1 - erfRC2 - erfRC3 + erfRC4);
}

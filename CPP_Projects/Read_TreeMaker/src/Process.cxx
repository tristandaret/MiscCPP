...
Process::Process(){
	...
	for(int i = 0; i < 32; i++){
		vmod_fph1f_XP.						push_back(new TH1F(Form("fph1f_XP_%d", i), Form("Energy loss in ERAM %i;dE/dx (ADC counts/cm);Count", channel2iD[i]), 100, 0, dEdxmax));
		vmod_fph1f_WF.						push_back(new TH1F(Form("fph1f_WF_%d", i), Form("Energy loss in ERAM %i;dE/dx (ADC counts/cm);Count", channel2iD[i]), 100, 0, dEdxmax));
		vmod_fph2f_XPtmean.					push_back(new TH2F(Form("fph2f_XPtmean_%d", i), Form("Energy loss (XP) vs mean time in ERAM %i;mean time (ns);dE/dx (ADC counts/cm)", channel2iD[i]), 510, 0, 510, 100, 0, dEdxmax));
		vmod_fph2f_XPdrift.					push_back(new TH2F(Form("fph2f_XPdrift_%d", i), Form("Energy loss (XP) vs drift time in ERAM %i;drift time (timebins);dE/dx (ADC counts/cm)", channel2iD[i]), 510, 0, 510, 100, 0, dEdxmax));
	}

	for(int i=0; i<nmombins;i++){
		int mommin =						i*mombinwidth - momrange;
		int mommax =						(i+1)*mombinwidth - momrange;
		vmom_fph1f_WF.						push_back(new TH1F(Form("fph1f_mom_WF_%d_%d", mommin, mommax), Form("Energy loss | %d < p < %d; dE/dx (ADC counts/cm); Count", mommin, mommax), 100, 0, dEdxmax));
		vmom_fph1f_XP.						push_back(new TH1F(Form("fph1f_mom_XP_%d_%d", mommin, mommax), Form("Energy loss | %d < p < %d; dE/dx (ADC counts/cm); Count", mommin, mommax), 100, 0, dEdxmax));
		vmom_fph1f_pullmu.					push_back(new TH1F(Form("fph1f_pull_%d_%d", mommin, mommax), Form("Muon pull | %d < p < %d; Pull; Count", mommin, mommax), 100, -20, 20));
		vmom_fph1f_pullelec.				push_back(new TH1F(Form("fph1f_pullelec_%d_%d", mommin, mommax), Form("Electron pull | %d < p < %d; Pull; Count", mommin, mommax), 100, -20, 20));
	}

	for(int i=0; i<nddbins;i++){
		int ddmin =							i*ddbinwidth;
		int ddmax =							(i+1)*ddbinwidth;
		vdd_fph1f_WF.						push_back(new TH1F(Form("ph1f_dd_WF_%d_%d", ddmin, ddmax), Form("Energy loss | %d < drift distance < %d; dE/dx (ADC counts/cm); Count", ddmin, ddmax), 100, 0, dEdxmax));
		vdd_fph1f_XP.						push_back(new TH1F(Form("ph1f_dd_XP_%d_%d", ddmin, ddmax), Form("Energy loss | %d < drift distance < %d; dE/dx (ADC counts/cm); Count", ddmin, ddmax), 100, 0, dEdxmax));
	}

	for(int i=0; i<nphibins;i++){
		int phimin =						i*phibinwidth - phirange;
		int phimax =						(i+1)*phibinwidth - phirange;
		vphi_fph1f_WF.						push_back(new TH1F(Form("fph1f_phi_WF_%d_%d", phimin, phimax), Form("Energy loss | %d < #varphi < %d; dE/dx (ADC counts/cm); Count", phimin, phimax), 100, 0, dEdxmax));
		vphi_fph1f_XP.						push_back(new TH1F(Form("fph1f_phi_XP_%d_%d", phimin, phimax), Form("Energy loss | %d < #varphi < %d; dE/dx (ADC counts/cm); Count", phimin, phimax), 100, 0, dEdxmax));
	}

	for(int i=0; i<nthetabins;i++){
		int thetamin =						i*thetabinwidth - thetarange;
		int thetamax =						(i+1)*thetabinwidth - thetarange;
		vtheta_fph1f_WF.					push_back(new TH1F(Form("fph1f_theta_WF_%d_%d", thetamin, thetamax), Form("Energy loss | %d < #theta < %d; dE/dx (ADC counts/cm); Count", thetamin, thetamax), 100, 0, dEdxmax));
		vtheta_fph1f_XP.					push_back(new TH1F(Form("fph1f_theta_XP_%d_%d", thetamin, thetamax), Form("Energy loss | %d < #theta < %d; dE/dx (ADC counts/cm); Count", thetamin, thetamax), 100, 0, dEdxmax));
		vtheta_fph1f_pullmu.				push_back(new TH1F(Form("fph1f_pullmu_%d_%d", thetamin, thetamax), Form("Muon pull | %d < #theta < %d; Pull; Count", thetamin, thetamax), 100, -20, 20));
		vtheta_fph1f_pullelec.				push_back(new TH1F(Form("fph1f_pullelec_%d_%d", thetamin, thetamax), Form("Electron pull | %d < #theta < %d; Pull; Count", thetamin, thetamax), 100, -20, 20));
	}
}
...
Process::~Process() {
	...
	for (auto ptr : vmom_fph1f_pullmu) delete ptr;
	for (auto ptr : vmom_fph1f_pullelec) delete ptr;
	...
	for (auto ptr : vtheta_fph1f_pullmu) delete ptr;
	for (auto ptr : vtheta_fph1f_pullelec) delete ptr;
...
void Process::SetCuts(){
	...
	// dxmin = 50, ddEdxmax = 150;			fcutslist += ("_" + std::to_string(dxmin) 		+ "dx" + std::to_string(ddEdxmax));

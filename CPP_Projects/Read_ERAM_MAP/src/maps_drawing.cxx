...
void Drawing::maps_pdf()
{
   ...
   for (int i = 0; i < 34; i++) {
      v_h2_gain.push_back(
         new TH2F(Form("h2_gain_%d", i), "", 36, -0.5, 35.5, 32, -0.5, 31.5));
      v_h2_rc.push_back(
         new TH2F(Form("h2_rc_%d", i), "", 36, -0.5, 35.5, 32, -0.5, 31.5));
   }
   ...
   for (int i = 0; i < 34; i++) {
      ...
      for (int j = 0; j < 36; j++) {
         for (int k = 0; k < 32; k++) {
            ...
            if (gain != 0)
               v_h2_gain[i]->Fill(j, k, gain);
            if (rc != 0)
               v_h2_rc[i]->Fill(j, k, rc);
            if (gain == 0 or rc == 0)
               std::cout << "ERAM#" << id << " (" << j << "," << k << ") has a hole"
                         << std::endl;
            // lowest values
            if (rc < low_rc and rc != 0)
               low_rc = rc;
            if (gain < low_gain and gain != 0)
               low_gain = gain;
            // highest values
            if (rc > high_rc)
               high_rc = rc;
      ...
      v_h2_gain[i]->SetTitle(
         Form("ERAM#%i (%i) Gain = %0.f", id, i, eram_gain / n_pads_eram));
      v_h2_rc[i]->SetTitle(
         Form("ERAM#%i (%i) RC = %0.f ns/mm^{2}", id, i, eram_rc / n_pads_eram));
   ...
   // v_avg_gain[i] << std::endl; for(int i=0;i<(int)v_avg_rc.size();i++)   std::cout
   // <<"RC   ERAM#" << std::setw(2) << pERAMMaps->ID(i) << "(" << std::setw(2) << i <<
   // "): " << std::setprecision(3) << v_avg_rc[i] << std::endl;
...
void Drawing::maps_csv()
{
   ...
   for (int i = 0; i < 34; i++) {
      std::cout << "RC   ERAM#" << std::setw(2) << pERAMMaps->ID(i) << "(" << std::setw(2)
                << i << "): " << std::setprecision(3) << v_avg_rc[i] << std::endl;
      std::ofstream csvFile(
         Form("Output/CSV/ERAM_%i_RC_grayscale.csv", pERAMMaps->ID(i)));
      for (int j = 0; j < 32; j++) {
         for (int k = 0; k < 36; k++) {
...
void Drawing::Distributions()
{
   ...
   TH2F *ph2_rcmap = new TH2F("ph2_rcmap", ";Column;Row;Average RC [ns/mm^{2}]", col,
                              -0.5, 35.5, row, -0.5, 31.5);
   TH2F *ph2_rcmapbot =
      new TH2F("ph2_rcmapbot", ";Column;Row; bottom HAT average RC [ns/mm^{2}]", col,
               -0.5, 35.5, row, -0.5, 31.5);
   TH2F *ph2_rcmaptop =
      new TH2F("ph2_rcmaptop", ";Column;Row; top HAT average RC [ns/mm^{2}]", col, -0.5,
               35.5, row, -0.5, 31.5);
   TH2F *ph2_rcdiffTPC =
      new TH2F("ph2_rcdiffTPC", ";Column;Row;RC(tHAT) - RC(bHAT) [ns/mm^{2}]", col, -0.5,
               35.5, row, -0.5, 31.5);

   TH2F *ph2_rcnorm =
      new TH2F("ph2_rcnorm", ";Column;Row;Normalized average RC [ns/mm^{2}]", col, -0.5,
               35.5, row, -0.5, 31.5);
   TH2F *ph2_rcnormbot =
      new TH2F("ph2_rcnormbot", ";Column;Row;Normalized average RC (bHAT) [ns/mm^{2}]",
               col, -0.5, 35.5, row, -0.5, 31.5);
   TH2F *ph2_rcnormtop =
      new TH2F("ph2_rcnormtop", ";Column;Row;Normalized average RC (tHAT) [ns/mm^{2}]",
               col, -0.5, 35.5, row, -0.5, 31.5);
   TH2F *ph2_rcnormdiffTPC =
      new TH2F("ph2_rcnormdiffTPC",
               "Absolute difference of tHAT vs bHAT normalized average RC map (in %)",
               col, -0.5, 35.5, row, -0.5, 31.5);
   ...
   float RCmin = 50, RCmax = 275;
   float nRC = RCmax - RCmin;
   TH2F *ph2_rcnormstd =
   ...
   std::cout << "Channel: ";
   for (int i = 0; i < (int)v_channel_low.size(); i++)
      std::cout << v_channel_low[i] << " ";
   std::cout << std::endl;
   std::cout << "ID:      ";
   for (int i = 0; i < (int)v_ID_low.size(); i++)
      std::cout << v_ID_low[i] << " ";
   std::cout << std::endl;
   std::cout << "High RC ERAMs:" << std::endl;
   std::cout << "Channel: ";
   for (int i = 0; i < (int)v_channel_high.size(); i++)
      std::cout << v_channel_high[i] << " ";
   std::cout << std::endl;
   std::cout << "ID:      ";
   for (int i = 0; i < (int)v_ID_high.size(); i++)
      std::cout << v_ID_high[i] << " ";
   std::cout << std::endl;
   std::cout << "RC:      ";
   for (int i = 0; i < (int)v_RCs.size(); i++)
      std::cout << v_RCs[i] << ", ";
   std::cout << std::endl;
   ...
   for (int i = 0; i < nerams; i++) {
      ...
      for (int j = 0; j < col; j++) {
         for (int k = 0; k < row; k++) {
            ...
            ph2_rcnorm->Fill(
               j, k, (rc - v_min_rc[i]) / (v_max_rc[i] - v_min_rc[i]) / (nerams - 1));
            csv_rcnorm[j][31 - k] +=
               (rc - v_min_rc[i]) / (v_max_rc[i] - v_min_rc[i]) / (nerams - 1);
            if (i < nerams / 2) {
               ph1_rcbot->Fill(rc);
               ph2_rcmapbot->Fill(j, k, rc / (nerams / 2 - 1));
               ph2_rcnormbot->Fill(j, k,
                                   (rc - v_min_rc[i]) / (v_max_rc[i] - v_min_rc[i]) /
                                      (nerams / 2 - 1));
               csv_rcnormbot[j][31 - k] +=
                  (rc - v_min_rc[i]) / (v_max_rc[i] - v_min_rc[i]) / (nerams / 2 - 1);
            } else {
               ph1_rctop->Fill(rc);
               ph2_rcmaptop->Fill(j, k, rc / (nerams / 2));
               ph2_rcnormtop->Fill(
                  j, k, (rc - v_min_rc[i]) / (v_max_rc[i] - v_min_rc[i]) / (nerams / 2));
               csv_rcnormtop[j][31 - k] +=
                  (rc - v_min_rc[i]) / (v_max_rc[i] - v_min_rc[i]) / (nerams / 2);
            }
            ...
            if (k > 0)
               ph1_rcdiffvert->Fill(rc - pERAMMaps->RC(i, j, k - 1));
            // Std of RC difference vertical and horizontal with cutoff for differences >
            // 20
            if (fabs(rc - pERAMMaps->RC(i, j - 1, k)) < 20)
               ph1_rcdiffcut->Fill(rc - pERAMMaps->RC(i, j - 1, k));
            if (fabs(rc - pERAMMaps->RC(i, j, k - 1)) < 20)
               ph1_rcdiffcut->Fill(rc - pERAMMaps->RC(i, j, k - 1));
         }
      }
   }
   ...
   doubleGaus->SetParameters(gausn1->GetParameter(0), gausn1->GetParameter(1),
                             gausn1->GetParameter(2), gausn2->GetParameter(0),
                             gausn2->GetParameter(1), gausn2->GetParameter(2));
   ...
   for (int j = 0; j < col; j++) {
      for (int k = 0; k < row; k++) {
         for (int i = 0; i < nerams; i++) {
            float rc_normed =
               (pERAMMaps->RC(i, j, k) - v_min_rc[i]) / (v_max_rc[i] - v_min_rc[i]);
   ...
   gPad->UseCurrentStyle();
   pTCanvas->cd();
   gPad->SetMargin(0.15, 0.03, 0.15, 0.03);
   ...
   pTLegend->AddEntry(ph1_rctop, "top HAT", "f");
   pTLegend->AddEntry(ph1_rcbot, "bottom HAT", "f");
   pTLegend->SetFillStyle(1001);
   pTLegend->SetBorderSize(2);
   pTLegend->SetTextColor(kBlue - 1);
   pTLegend->SetLineWidth(2);
   ...
   PrintResolution(ph1_rchigh, pTCanvas, 0.65, 0.65, kRed + 1, "High RC ERAMs");
   PrintResolution(ph1_rclow, pTCanvas, 0.65, 0.3, kBlue + 1, "Low RC ERAMs");
   ...
   paveTextRCDiffToRefHigh.AddText("High RC ERAMs");
   paveTextRCDiffToRefHigh.AddText(Form(
      "#mu = %.1f, #sigma = %.1f", ph1_rchighdiff->GetMean(), ph1_rchighdiff->GetRMS()));
   ...
   paveTextRCDiffToRefLow.AddText("Low RC ERAMs");
   paveTextRCDiffToRefLow.AddText(Form(
      "#mu = %.1f, #sigma = %.1f", ph1_rclowdiff->GetMean(), ph1_rclowdiff->GetRMS()));
   ...
   TPaveText pTPaveNeigh(0.64, 0.6, 0.93, 0.68, "NDC");
   pTPaveNeigh.SetTextColor(kMagenta - 2);
   pTPaveNeigh.SetLineWidth(2);
   pTPaveNeigh.SetBorderSize(2);
   pTPaveNeigh.SetTextAlign(12);
   pTPaveNeigh.SetLineColor(kMagenta + 1);
   pTPaveNeigh.AddText(Form("#mu = %.1f, #sigma = %.1f", ph1_rclowdiff->GetMean(),
                            ph1_rclowdiff->GetRMS()));
   pTPaveNeigh.Draw();
   // Add legend
   pTLegend = new TLegend(0.64, 0.7, 0.93, 0.89);
   ...
   pTCanvas->SetCanvasSize(pTCanvas->GetWw(), 1200);
   gStyle->SetPadRightMargin(0.12);
   pTCanvas->Clear();
   pTCanvas->Divide(4, 4, 0.0, 0.0);
   for (std::vector<int> vEndplate : {vSouth, vNorth}) {
      for (int i = 0; i < 16; i++) {
         ...
         pavetextMod.AddText(v_ph2_GRC[eramIndex]->GetTitle());

         v_ph2_GRC[eramIndex]->SetTitle("");
         v_ph2_GRC[eramIndex]->SetTitleSize(0.1, "XY");
         v_ph2_GRC[eramIndex]->SetLabelSize(0.1, "XY");
         v_ph2_GRC[eramIndex]->Draw("HIST");

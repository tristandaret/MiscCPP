...
void Drawing::maps_pdf()
{
   ...
   for (int i = 0; i < 34; i++) {
      ...
      for (int j = 0; j < 36; j++) {
         for (int k = 0; k < 32; k++) {
            ...
            if (gain == 0 or rc == 0)
               std::cout << "ERAM#" << id << " (" << j << "," << k << ") has a hole" << std::endl;
            // lowest values
   ...
   // for(int i=0;i<(int)v_avg_gain.size();i++) std::cout <<"Gain ERAM#" << std::setw(2) << pERAMMaps->ID(i) << "(" <<
   // std::setw(2) << i << "): " << std::setprecision(4) << v_avg_gain[i] << std::endl; for(int
   // i=0;i<(int)v_avg_rc.size();i++)   std::cout <<"RC   ERAM#" << std::setw(2) << pERAMMaps->ID(i) << "(" <<
   // std::setw(2) << i << "): " << std::setprecision(3) << v_avg_rc[i] << std::endl;
...
void Drawing::maps_csv()
{
   ...
   for (int i = 0; i < 34; i++) {
      std::cout << "RC   ERAM#" << std::setw(2) << pERAMMaps->ID(i) << "(" << std::setw(2) << i
...
void Drawing::Distributions()
{
   ...
   TH2F *ph2_GRC =
      new TH2F("ph2_GRC", "Gain vs RC;RC value (ns/mm^{2});Gain value (ADC count)", 50, 50, 270, 50, 700, 2700);
   ...
   for (int i = 0; i < nerams; i++)
      v_ph2_GRC.push_back(
         new TH2F(Form("ph2_GRC_%d", i),
                  Form("ERAM#%i Gain vs RC;RC value (ns/mm^{2});Gain value (ADC count)", pERAMMaps->ID(i)), 50, 50, 270,
                  50, 700, 2700));
   ...
   TH2F *ph2_rcdiffTPC = new TH2F("ph2_rcdiffTPC", "Difference of tHAT vs bHAT average RC map;;;ns/mm^{2}", col, -0.5,
                                  35.5, row, -0.5, 31.5);

   TH2F *ph2_rcnorm = new TH2F("ph2_rcnorm", "Normalized average RC map", col, -0.5, 35.5, row, -0.5, 31.5);
   TH2F *ph2_rcnormbot =
      new TH2F("ph2_rcnormbot", "Normalized average RC map (bHAT)", col, -0.5, 35.5, row, -0.5, 31.5);
   TH2F *ph2_rcnormtop =
      new TH2F("ph2_rcnormtop", "Normalized average RC map (tHAT)", col, -0.5, 35.5, row, -0.5, 31.5);
   TH2F *ph2_rcnormdiffTPC =
      new TH2F("ph2_rcnormdiffTPC", "Absolute difference of tHAT vs bHAT normalized average RC map (in %)", col, -0.5,
               35.5, row, -0.5, 31.5);
   ...
   TH2F *ph2_rcnormstd =
      new TH2F("ph2_rcnormstd", "Std of normalized average RC difference map (in %)", col, -0.5, 35.5, row, -0.5, 31.5);
   ...
   TH1F *ph1_rcdiff =
      new TH1F("ph1_rcdiff", "RC difference between neighbours;RC difference (ns/mm^{2});Count", 61, -30, 30);
   TH1F *ph1_rcdiffcut =
      new TH1F("ph1_rcdiffcut", "RC difference between neighbours;RC difference (ns/mm^{2});Count", 61, -30, 30);
   ...
   pLatexRCDiff->SetNDC();
   pLatexRCDiff->DrawLatex(0.95, 0.65,
                           Form("#mu #pm #sigma = %.2f #pm %.2f", ph1_rcdiffcut->GetMean(), ph1_rcdiffcut->GetRMS()));
   // Add legend

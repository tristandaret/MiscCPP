...
float LUTMaker::LUTValues[LUTMaker::SNSTEPS_TRANS][LUTMaker::SNSTEPS_RC][LUTMaker::SNSTEPS_DRIFT][LUTMaker::SNSTEPS_D][LUTMaker::SNSTEPS_PHI];

// Constructor
LUTMaker::LUTMaker(){
	v_phi =	linspace(0, 90, SNSTEPS_PHI);
	v_impact =	linspace(0, fdiag/2, SNSTEPS_D);
	v_drift =	linspace(0, 1000, SNSTEPS_DRIFT);
	v_RC =	{112, 158};
	v_impactt =	{310/pow(10, 3.5), 350/pow(10, 3.5)};
...
void LUTMaker::DrawLengthMap(){
	TH2F h2("h2", "Length map", v_phi.size(), 0, 90, v_impact.size(), 0, fdiag/2);
...
void LUTMaker::MakeLUT(){
	...
	for(int itransvDiff = 0; itransvDiff < SNSTEPS_TRANS; itransvDiff++){
		transvDiff = v_impactt[itransvDiff];
		...
		for(int iRC = 0; iRC < SNSTEPS_RC; iRC++){
			...
			for(int idrift = 0; idrift < SNSTEPS_DRIFT; idrift++){
				...
				for(int iphi = 0; iphi < SNSTEPS_PHI; iphi++){
					...
					for(int id = 0; id < SNSTEPS_D; id++){
						...
						fp_trackmodel->ComputeAmplitudeLoss(length, d, phi, RC, drift, transvDiff);
				...
				LUT_log << "transvDiff " << transvDiff << " RC " << RC << " drift " << drift << " => " << duration_drift.count() << " s" << std::endl;
			}
			...
			LUT_log << "transvDiff " << transvDiff << " RC " << RC << " => " << duration_RC.count() << " s" << std::endl;
		}
		auto end_transvDiff = std::chrono::high_resolution_clock::now();
		std::chrono::duration<double> duration_transvDiff = end_transvDiff - start_transvDiff;
		time += duration_transvDiff.count();
		LUT_log << "transvDiff " << transvDiff << " => " << duration_transvDiff.count() << " s" << std::endl;
	}
...
void LUTMaker::LoadLUT(std::string LUTpath){
	...
	for (int i = 0; i < ptree.GetEntries(); i++) {
		...
		// std::cout << transvDiff << " " << RC << " " << drift << " " << d << " " << phi << " " << scalefactor << std::endl;
		// std::cout << itransvDiff << " " << iRC << " " << idrift << " " << id << " " << iphi << std::endl;
...
// Get factor from LUT
float LUTMaker::GetFactorFromLUT(const double &transvDiff, const double &RC, const double &drift, const double &d, const double &phi){ // keep double
	...
	if(idrift_min == idrift_max)w_drift =	1;
	else						w_drift =	1 - (idrift - idrift_min)/(idrift_max - idrift_min);
	...
	std::cout << "drift = " << drift << " | idrift_min = " << idrift_min << " | idrift = " << idrift << " | idrift_max = " << idrift_max << " | w_drift = " << w_drift << std::endl;
	std::cout << "d =     " << d << " | id_min = " << id_min << " | id = " << id << " | id_max = " << id_max << " | w_d = " << w_d << std::endl;
	std::cout << "phi =   " << phi << " | iphi_min = " << iphi_min << " | iphi = " << iphi << " | iphi_max = " << iphi_max << " | w_phi = " << w_phi << std::endl;

	// Interpolation
	float factor = 0;
	factor +=   w_drift   *   w_d   *   w_phi   * LUTValues[itransvDiff][iRC][(int)idrift_min][(int)id_min][(int)iphi_min];
	factor +=   w_drift   *   w_d   * (1-w_phi) * LUTValues[itransvDiff][iRC][(int)idrift_min][(int)id_min][(int)iphi_max];
	factor +=   w_drift   * (1-w_d) *   w_phi   * LUTValues[itransvDiff][iRC][(int)idrift_min][(int)id_max][(int)iphi_min];
	factor +=   w_drift   * (1-w_d) * (1-w_phi) * LUTValues[itransvDiff][iRC][(int)idrift_min][(int)id_max][(int)iphi_max];
	factor += (1-w_drift) *   w_d   *   w_phi   * LUTValues[itransvDiff][iRC][(int)idrift_max][(int)id_min][(int)iphi_min];
	factor += (1-w_drift) *   w_d   * (1-w_phi) * LUTValues[itransvDiff][iRC][(int)idrift_max][(int)id_min][(int)iphi_max];
	factor += (1-w_drift) * (1-w_d) *   w_phi   * LUTValues[itransvDiff][iRC][(int)idrift_max][(int)id_max][(int)iphi_min];
	factor += (1-w_drift) * (1-w_d) * (1-w_phi) * LUTValues[itransvDiff][iRC][(int)idrift_max][(int)id_max][(int)iphi_max];

	std::cout << std::endl;
	std::cout << LUTValues[itransvDiff][iRC][(int)idrift_min + 0][(int)id_min + 1][(int)iphi_min + 1] << "  " << LUTValues[itransvDiff][iRC][(int)idrift_min + 1][(int)id_min + 1][(int)iphi_min + 1] << std::endl;
	std::cout << LUTValues[itransvDiff][iRC][(int)idrift_min + 0][(int)id_min + 0][(int)iphi_min + 1] << "  " << LUTValues[itransvDiff][iRC][(int)idrift_min + 1][(int)id_min + 0][(int)iphi_min + 1] << std::endl;
	std::cout << std::endl;
	std::cout << LUTValues[itransvDiff][iRC][(int)idrift_min + 0][(int)id_min + 1][(int)iphi_min + 0] << "  " << LUTValues[itransvDiff][iRC][(int)idrift_min + 1][(int)id_min + 1][(int)iphi_min + 0] << std::endl;
	std::cout << LUTValues[itransvDiff][iRC][(int)idrift_min + 0][(int)id_min + 0][(int)iphi_min + 0] << "  " << LUTValues[itransvDiff][iRC][(int)idrift_min + 1][(int)id_min + 0][(int)iphi_min + 0] << std::endl;
	std::cout << std::endl;
...
void LUTMaker::DrawDiffInterpolExact(	const double &transvDiff, const double &RC, const double &drift,
	...
	for (int i = 0; i < nbins; i++) {
		for (int j = 0; j < nbins; j++) {
			...
			double L = arr_length[(int)std::round(phi/sSTEP_PHI)][(int)std::round(d/sSTEP_IMPACT)];
			float scalefactor = GetFactorFromLUT(transvDiff, RC, drift, d, phi);
			if(L < 2){
				h2_diff.SetBinContent(h2_diff.FindBin(phi, d), -10);
				continue;
			}
			fp_trackmodel->ComputeAmplitudeLoss(L, d, phi, v_RC[0], v_drift[0], v_impactt[0]);
	...
	h2_diff.Draw("colz");
	c1->SaveAs(Form("Output_PDF/diff_interpol_exact_%dphi%d_%dd%d_%dbins.pdf", (int)phimin, (int)phimax, (int)dmin, (int)dmax, nbins));
}
...
public:
    THATReconTreeMakerLoop() : fEvent(0),
                               fMomentum(0),
                               fNclusters(0),
                               fCurvature(0),
                               fDirNorm(0) {
    ...
    bool operator()(ND::TND280Event &event) override {
        ...
        // Initialize inal vectors of subtracks
        std::vector<std::vector<float>> arr_tracks_x;
        std::vector<std::vector<float>> arr_tracks_y;
        std::vector<std::vector<float>> arr_tracks_z;
        std::vector<std::vector<float>> arr_tracks_ex;
        std::vector<std::vector<float>> arr_tracks_ey;
        std::vector<std::vector<float>> arr_tracks_ez;
        std::vector<int>                v_MM_ID;
        int n_tracks                    = 0 ;
        for (auto const &pattern: *fit->GetResultsContainer("THATPID")) {
            ...
            for (auto const &constituent: *pid_result->GetConstituents()) {
                ...
                // Clusters (working)
                std::vector<std::vector<float>> arr_subtrack_x;
                std::vector<std::vector<float>> arr_subtrack_y;
                std::vector<std::vector<float>> arr_subtrack_z;
                std::vector<std::vector<float>> arr_subtrack_ex;
                std::vector<std::vector<float>> arr_subtrack_ey;
                std::vector<std::vector<float>> arr_subtrack_ez;
                for(int k = 0 ; k < 32 ; k++){
                    std::vector<float> v_cluster_x ;
                    std::vector<float> v_cluster_y ;
                    std::vector<float> v_cluster_z ;
                    std::vector<float> v_cluster_ex ;
                    std::vector<float> v_cluster_ey ;
                    std::vector<float> v_cluster_ez ;
                    arr_subtrack_x.push_back(v_cluster_x);
                    arr_subtrack_y.push_back(v_cluster_y);
                    arr_subtrack_z.push_back(v_cluster_z);
                    arr_subtrack_ex.push_back(v_cluster_ex);
                    arr_subtrack_ey.push_back(v_cluster_ey);
                    arr_subtrack_ez.push_back(v_cluster_ez);
                }
                ...
                for (auto clus: *trk->GetConstituents()) {
                    ...
                    TVector3 global;
                    if( clt->GetPosition().X() > 0 )    global.SetXYZ( 980.59, clt->GetPosition().Y(), clt->GetPosition().Z() ); // X is the drift
                    else                                global.SetXYZ(-980.59, clt->GetPosition().Y(), clt->GetPosition().Z() );
                    if (clt->GetPosition().Y() > 1200 or clt->GetPosition().Y() < 400 or clt->GetPosition().Z() > -1000 or clt->GetPosition().Z() < -2900) continue;
                    ND::TGeometryId id;
                    bool is_inside = ND::TGeomInfo::HAT().GlobalXYZToGeomId(global, id);
                    if (!is_inside){ ND280Log("ID not found"); continue; }
                    auto mms  = ND::TGeomInfo::HAT().GeomIdToMM(id);
                    auto half = ND::TGeomInfo::HAT().GeomIdToHalf(id);
                    arr_subtrack_x[mms+8*half].push_back(clt->GetPosition().X()) ;
                    arr_subtrack_y[mms+8*half].push_back(clt->GetPosition().Y()) ;
                    arr_subtrack_z[mms+8*half].push_back(clt->GetPosition().Z()) ;
                    arr_subtrack_ex[mms+8*half].push_back(clt->GetPositionVariance().X()) ;
                    arr_subtrack_ey[mms+8*half].push_back(clt->GetPositionVariance().Y()) ;
                    arr_subtrack_ez[mms+8*half].push_back(clt->GetPositionVariance().Z()) ;
                ...
                for(int iMM = 0 ; iMM < 32 ; iMM++){
                    if(list_subtrack[iMM] == 0) continue;
                    arr_tracks_x.       push_back(arr_subtrack_x[iMM]) ;
                    arr_tracks_y.       push_back(arr_subtrack_y[iMM]) ;
                    arr_tracks_z.       push_back(arr_subtrack_z[iMM]) ;
                    arr_tracks_ex.      push_back(arr_subtrack_ex[iMM]) ;
                    arr_tracks_ey.      push_back(arr_subtrack_ey[iMM]) ;
                    arr_tracks_ez.      push_back(arr_subtrack_ez[iMM]) ;
                    v_MM_ID.            push_back(iMM) ;
                }
                n_tracks += n_subtracks ;
        ...
        ND280Log("    Subtracks: " << n_tracks) ;
        int i_track = 0 ;
        for(int isub = 0 ; isub < n_tracks ; isub++){
            ND280Log("    Open:  Subtracks " << i_track+1 << "/" << n_tracks << " (MM #" << v_MM_ID[isub] << "):") ;
            for(int iC = 0 ; iC < (int)arr_tracks_x[isub].size() ; iC++){
                ND280Log("      Cluster " << std::setw(3) << iC <<  ": " << 
                            "X = " << arr_tracks_x[isub][iC] << " ± " << arr_tracks_ex[isub][iC] << " | " <<
                            "Y = " << arr_tracks_y[isub][iC] << " ± " << arr_tracks_ey[isub][iC] << " | " <<
                            "Z = " << arr_tracks_z[isub][iC] << " ± " << 0.3 );
            }
            ND280Log("    Close: Subtracks " << i_track+1 << "/" << n_tracks << " (MM #" << v_MM_ID[isub] << "):") ;
            i_track++;
        }
    ...
    void Usage() override {
        std::cout << "Generate TTree from reconstructed track parameters."
                  << std::endl
                  << std::endl;
        fParOptMan.Usage();
    }
    ...
    void ComputeResolution(ND::THandle<ND::TReconTrack> trk) {
        ...
        for (auto clus: *trk->GetConstituents()) {
            ...
            if (clt) {
                ...
                if (clt->GetPosition().Y() > 1200 or clt->GetPosition().Y() < 400 or clt->GetPosition().Z() > -1000 or clt->GetPosition().Z() < -2900)
                    continue;
                ...
                auto r = sqrt((clt->GetPosition().Z() - fcenter_circle_X) *
                              ...
                              (clt->GetPosition().Y() - fcenter_circle_Y)) - R;

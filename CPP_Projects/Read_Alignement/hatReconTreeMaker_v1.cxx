            ...
            for (auto const &constituent: *pid_result->GetConstituents()) {
                ...
                */
                ...
                for (auto clus: *trk->GetConstituents()) {
                    ...
                    n_clus[mms+8*half] ++ ;
                    // ND280Log("    Cluster " << std::setw(3) << n_clus <<  ": " << "MM " << std::setw(2) << mms+8*half << " | col: " << std::setw(2) << col << " | row: " << std::setw(2) << row << " | " << 
                ...
                //     if( chit->GetPosition().X() > 0 )   global_clus.SetXYZ(chit->GetPosition().X(), chit->GetPosition().Y(), chit->GetPosition().Z() );
                //     else                                global_clus.SetXYZ(chit->GetPosition().X(), chit->GetPosition().Y(), chit->GetPosition().Z() );
                //     if (chit->GetPosition().Y() > 1200 or chit->GetPosition().Y() < 400 or chit->GetPosition().Z() > -1000 or chit->GetPosition().Z() < -2900) continue;
                //     ND::TGeometryId id;
                //     bool is_inside = ND::TGeomInfo::HAT().GlobalXYZToGeomId(global_clus, id);
                //     if (!is_inside) { ND280Log("ID not found"); continue; }
                //     auto row = ND::TGeomInfo::HAT().GeomIdToRow(id);
                //     auto col = ND::TGeomInfo::HAT().GeomIdToColumn(id);
                //     auto mms = ND::TGeomInfo::HAT().GeomIdToMM(id);
                //     ND280Log(       "Cluster " << std::setw(3) << n_clus <<  ": " << 
                //                     "X = " << chit->GetPosition().X()   << " ± " << std::setw(5) << std::setprecision(4) << chit->GetUncertainty().X() << " | " <<
                //                     "Y = " << chit->GetPosition().Y()   << " ± " << std::setw(5) << std::setprecision(4) << chit->GetUncertainty().Y() << " | " <<
                //                     "Z = " << chit->GetPosition().Z()   << " ± " << std::setw(5) << std::setprecision(4) << chit->GetUncertainty().Z() << " | " <<
                //                     "Endplate: " << fEndPlate_ID << " | MM " << mms << " | col: " << col << " | row: " << row);
                ...
                //     // Clusters (ComboHit)
                //     TVector3 global_clus;
                //     if( chit->GetPosition().X() > 0 )   global_clus.SetXYZ( 980.59, chit->GetPosition().Y(), chit->GetPosition().Z() );
                //     else                                global_clus.SetXYZ(-980.59, chit->GetPosition().Y(), chit->GetPosition().Z() );
                //     if (chit->GetPosition().Y() > 1200 or chit->GetPosition().Y() < 400 or chit->GetPosition().Z() > -1000 or chit->GetPosition().Z() < -2900) continue;
                //     ND::TGeometryId id;
                //     bool is_inside = ND::TGeomInfo::HAT().GlobalXYZToGeomId(global_clus, id);
                //     if (!is_inside) { ND280Log("ID not found"); continue; }
                ...
                //     auto col = ND::TGeomInfo::HAT().GeomIdToColumn(id);
                //     auto mms = ND::TGeomInfo::HAT().GeomIdToMM(id);
                //     ND280Log("Cluster " << n_clus << " | MM " << mms << " | Endplate: " << fEndPlate_ID     << " | col: " << col << " | row: " << row << " | " << 
                ...
                //         TVector3 global_pad;
                //         if (hit->GetPosition().X() > 0) global_pad.SetXYZ( 980.59, hit->GetPosition().Y(), hit->GetPosition().Z());
                //         else                            global_pad.SetXYZ(-980.59, hit->GetPosition().Y(), hit->GetPosition().Z());

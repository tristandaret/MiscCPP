...
void Reader(const char* filename) {
    ...
    RunT* runT = nullptr;
    treeT->SetBranchAddress("events", &runT);
    ...
    for (int entry = 0; entry < treeT->GetEntries(); entry++) {
        ...
        const int n_evt = runT->events.size();

        int i_evt = 0;
        for (const EventT& event : runT->events) {
            std::cout << "Event's ID: #" << event.eventid << endl;
            int i_subtrk = 0;

            for (const TrackT& track : event.tracks) {
                std::cout << "  Track's ID: #" << track.trackid << " | " << track.n_subtracks << " subtracks" << endl;
                std::cout << "  Track's parameters (mm): X = " << track.params[0] << " | Y = " << track.params[1] << " | Z = " << track.params[2] <<
                        " | Vx = " << track.params[3] << " | Vy = " << track.params[4] << " | Vz = " << track.params[5] << 
                        " | Curve = " << track.params[6] << endl;
                ...
                for (const SubtrackT& subtrack : track.subtracks) {
                    ...
                    for (const ClusterT& cluster : subtrack.clusters) {
                        std::cout << "      Cluster's position: X = " << cluster.x << " | Y = " << cluster.y << " | Z = " << cluster.z << endl;
                    ...
                    if(avg_x > 0)   tg_subtrk->SetMarkerStyle(20);
                    else            tg_subtrk->SetMarkerStyle(24);
                    tg_subtrk->GetXaxis()->SetLimits(-3000, -1800) ;
                    tg_subtrk->SetMinimum(300);
                    tg_subtrk->SetMaximum(1200);
                    if(i_subtrk == 0){
                        tg_subtrk->     SetNameTitle(Form("tg_evt%i", event.eventid),Form("Event %i Display;Z (mm);Y (mm)", event.eventid));
                        tg_subtrk->     DrawClone("ap");
                    } else tg_subtrk->  DrawClone("p same");
                    delete tg_subtrk;
                    tg_subtrk = nullptr;
                    i_subtrk++;
                }
            ...
            if(i_evt == 0)            pTCanvas->SaveAs(OutputFile_Beg.c_str()) ;
            else if(i_evt == n_evt-1) pTCanvas->SaveAs(OutputFile_End.c_str()) ;
